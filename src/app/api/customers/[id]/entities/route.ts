  ...
  try {
    ...
    try {
      const connectionsResponse = await fetch(
        `${MEMBRANE_API_URI}/connections?userId=${customer.internalId}`,
        {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${adminToken}`,
      ...
      if (connectionsResponse.ok) {
        ...
        for (const connection of connections) {
          const deleteResponse = await fetch(
            `${MEMBRANE_API_URI}/connections/${connection.id}`,
            {
              method: 'DELETE',
              headers: {
                'Authorization': `Bearer ${adminToken}`,
    ...
    try {
      const flowInstancesResponse = await fetch(
        `${MEMBRANE_API_URI}/flow-instances?userId=${customer.internalId}`,
        {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${adminToken}`,
      ...
      if (flowInstancesResponse.ok) {
        ...
        for (const flowInstance of flowInstances) {
          const deleteResponse = await fetch(
            `${MEMBRANE_API_URI}/flow-instances/${flowInstance.id}`,
            {
              method: 'DELETE',
              headers: {
                'Authorization': `Bearer ${adminToken}`,
    ...
    return NextResponse.json(
      { error: 'Failed to delete customer entities' },
      { status: 500 }
    );
  }
}
