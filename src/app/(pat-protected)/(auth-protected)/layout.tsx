  ...
  return (
    <SidebarInset className={cn('overflow-hidden flex flex-col', CONTENT_HEIGHT)}>
      ...
      <FloatingPortalBoundary className="flex-1 flex flex-col min-h-0">
        <div className="flex flex-1 flex-col gap-4 p-4 pt-0 overflow-auto min-h-0">{children}</div>
      </FloatingPortalBoundary>
    </SidebarInset>
  );
}

export default function UserAuthProtectedLayout({ children }: { children: React.ReactNode }) {
  return (
